...
_UNLIMITED_EXPIRY = "9999-12-31T23:59:59Z"
_PLACEHOLDER_VALUES = {"", "ဒီနေရာ", "replace-me", "changeme"}
...
if ":" not in BOT_TOKEN:
    raise RuntimeError("BOT_TOKEN must be a Telegram bot token containing a colon.")
...
if not ADMIN_ID.lstrip("-").isdigit():
    raise RuntimeError("ADMIN_ID must be a numeric Telegram chat/user ID.")
...
async def web_server():
    ...
    try:
        ...
        raise RuntimeError("BOT_PORT must be an integer.") from exc
    if not 1 <= port <= 65535:
        raise RuntimeError("BOT_PORT must be between 1 and 65535.")
...
# ── Helper functions ───────────────────────────────────────────────────────
_DURATION_TOKEN_RE = re.compile(r"(\d+)\s*(mo|min|d|h|m)", re.IGNORECASE)
...
def _parse_duration_seconds(value):
    ...
    if normalized in {"unlimit", "unlimited"}:
        return float("inf")
    if not normalized:
        return None

    matches = list(_DURATION_TOKEN_RE.finditer(normalized))
    reconstructed = "".join(match.group(0).replace(" ", "") for match in matches)
    if not matches or reconstructed != normalized:
        return None
    ...
    return sum(
        int(match.group(1)) * multipliers[match.group(2).lower()]
...
def check_key_expiration(expiration_time):
    ...
    try:
        ...
        if expiration_time == _UNLIMITED_EXPIRY:
            return True
        if not isinstance(expiration_time, str) or not expiration_time.strip():
            return False

        raw = expiration_time.strip()
        try:
...
def _remember_scan(chat_id, mode, target, plan_filters):
    last_scan_params[chat_id] = {
        "mode": mode,
        "target": target,
        "plan_filters": list(plan_filters or []),
    }
    save_state()


def _remove_scan_if_current(chat_id, scan_id):
    ...
    try:
        while True:
            ...
            if current_task.get("stop"):
                ...
                _remove_scan_if_current(chat_id, scan_id)
                return
...
async def start_brute_scan(chat_id, mode, target, original_message, plan_filters=None):
    ...
    scan_tasks[chat_id] = {
        ...
        "mode": mode,
        "target": target,
        "plan_filters": list(plan_filters),
    }
...
async def stop_scan(message):
    ...
    if data and not data["task"].done():
        ...
        _remember_scan(
            ...
            data["mode"],
            data.get("target"),
            data.get("plan_filters", []),
        )
        data["stop"] = True
        data["task"].cancel()
        _remove_scan_if_current(chat_id, data["scan_id"])
